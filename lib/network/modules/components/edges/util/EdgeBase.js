  ...
  drawArrowHead(ctx, position, viaNode, selected, hover) {
    ...
    if (node1 != node2) {
      ...
      if (this.options.csNodeAlign.enabled) {
          ...
          ctx.beginPath();
          ctx.moveTo((arrowPos.x + 1.1 * length * Math.cos(angle)), (arrowPos.y + 1.1 * length * Math.sin(angle)));
          ctx.lineTo((arrowPos.x - 1.1 * length * Math.cos(angle)), (arrowPos.y - 1.1 * length * Math.sin(angle)));
          ctx.stroke();
          ctx.restore();
